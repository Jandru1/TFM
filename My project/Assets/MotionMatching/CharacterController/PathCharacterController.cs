...
namespace MotionMatching
{
    ...
    public class PathCharacterController : MotionMatchingCharacterController
    {
        ...
        protected override void OnUpdate()
        {
            ...
            {
                Debug.Log("AveragedDeltaTime = " + AveragedDeltaTime);
                Debug.Log("TrajectoryPosPredictionFrames[i] = " + TrajectoryPosPredictionFrames[i]);
                SimulatePath(AveragedDeltaTime * TrajectoryPosPredictionFrames[i], CurrentKeyPoint, CurrentKeyPointT, //AveragedDeltaTime = 0.01666
                             out _, out _,
                                  ...
                                  out float2 nextPos, out float2 nextDir)
        {
            ...
            {
                Debug.Log("CurrentKP = " + currentKeypoint);
                KeyPoint current = Path[currentKeypoint];
                Debug.Log("current (position) = " + current.Position);
                Debug.Log("Path length = " + Path.Length);
                KeyPoint next = Path[(currentKeypoint + 1) % Path.Length];
                Debug.Log("Path[kp+1]%p.lenth = " + next.Position);
                float2 dir = next.Position - current.Position;
                Debug.Log("dir = " + dir);
                ...
                float dt = math.min(remainingTime, timeToNext); //Minimo entre lo que tardas a llegar al destino del path y el remaining time
                remainingTime -= dt;
                if (remainingTime <= 0)
                {
                    // Move
                    currentPos += dirNorm * current.Velocity * dt;
                    Debug.Log("new currentPos = " + currentPos);
                    currentKeyPointT = math.distance(current.Position, currentPos) / math.distance(current.Position, next.Position);
                    Debug.Log("currentKeyPoint_T= " + currentKeyPointT);
                    nextPos = currentPos;
                    nextDir = dirNorm;
                }
                else
