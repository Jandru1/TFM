...
namespace RVO
{
    ...
    public class Simulator
    {
        ...
        private class Worker
        {
            ...
            {
                foreach (Agent a in _agents) a.update();
		...
		//private int lastNumAgents_;
		private int newAgentID_;
        private int newAgentWorkerID_;

        public int newObstacleID_;
		...
		private void addAgent(Agent agent) {
			agent.id_ = newAgentID_++;
			agents_[agent.id_] = agent;

            //Assign a worker to this agent
            workers_[newAgentWorkerID_].addAgent(agent);
            agent.workerID_ = newAgentWorkerID_;
            newAgentWorkerID_ = (newAgentWorkerID_ + 1) % workers_.Length;
		}

		public void removeAgent(int agentNo) {
            Agent a = agents_[agentNo];
            workers_[a.workerID_].RemoveAgent(a);
			agents_.Remove(agentNo);
		}

        public bool hasAgent(int agentNo)
        {
            return agents_.ContainsKey(agentNo);
        }
         ...
         * </remarks>
         */
        public List<int> addObstacle(IList<Vector2> vertices)
        {
            ...
            int firstObstacleId = newObstacleID_;

            for (int i = 0; i < vertices.Count; ++i)
            {
                int obstacleNo = newObstacleID_++;//obstacles_.Count;
                ...
                {
                    obstacle.next_ = obstacles_[firstObstacleId];
                ...
                obstacleIds.Add(obstacle.id_);
            }

            return obstacleIds;
        }

        public void RemoveObstacle(List<int> obstacleIds)
        {
            foreach (int obstacleId in obstacleIds)
            {
                if (obstacles_.ContainsKey(obstacleId))
                {
                    if (obstacles_[obstacleId].splitted_ && obstacles_.ContainsKey(obstacles_[obstacleId].splitObstacleId_))
                        obstacles_.Remove(obstacles_[obstacleId].splitObstacleId_);
                    obstacles_.Remove(obstacleId);
                }
            }
        }
        ...
        public void SetNumWorkers(int numWorkers)
        {
            ...
            //Assign the current agents to a worker
            int w = 0;
            foreach (var pair in agents_)
            {
                workers_[w].addAgent(pair.Value);
                pair.Value.workerID_ = w;
                w = (w + 1) % workers_.Length;
            }
        }
