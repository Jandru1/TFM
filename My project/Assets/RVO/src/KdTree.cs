...
namespace RVO
{
    ...
    internal class KdTree
    {
		...
		private Dictionary<int, Agent> agents_;
		private List<int> agentsKeys_;
        ...
        internal void buildAgentTree()
        {
			...
			if (agents_ == null || agents_.Count != numAgents)
            {
				agents_ = new Dictionary<int, Agent>();
				agentsKeys_ = new List<int>();
				foreach (var pair in Simulator.Instance.agents_) 
                {
					agents_[pair.Key] = pair.Value;
					agentsKeys_.Add(pair.Key);
                }

				agentTree_ = new AgentTreeNode[2 * numAgents];
			...
			if (numAgents != 0)
            {
				buildAgentTreeRecursive(0, numAgents, 0);
            }
        }
        ...
        internal void buildObstacleTree()
        {
            ...
            foreach (KeyValuePair<int,Obstacle> obstaclePair in Simulator.Instance.obstacles_)
            {
                //obstacles.Add(Simulator.Instance.obstacles_[i]);
                obstacles.Add(obstaclePair.Value);
            }
        ...
        private void buildAgentTreeRecursive(int begin, int end, int node)
        {
            ...
            agentTree_[node].minX_ = agentTree_[node].maxX_ = agents_[agentsKeys_[begin]].position_.x_;
			agentTree_[node].minY_ = agentTree_[node].maxY_ = agents_[agentsKeys_[begin]].position_.y_;

            for (int i = begin + 1; i < end; ++i)
            {
				agentTree_[node].maxX_ = Math.Max(agentTree_[node].maxX_, agents_[agentsKeys_[i]].position_.x_);
				agentTree_[node].minX_ = Math.Min(agentTree_[node].minX_, agents_[agentsKeys_[i]].position_.x_);
				agentTree_[node].maxY_ = Math.Max(agentTree_[node].maxY_, agents_[agentsKeys_[i]].position_.y_);
				agentTree_[node].minY_ = Math.Min(agentTree_[node].minY_, agents_[agentsKeys_[i]].position_.y_);
            }
            ...
            if (end - begin > MAX_LEAF_SIZE)
            {
                ...
                while (left < right)
                {
					while (left < right && (isVertical ? agents_[agentsKeys_[left]].position_.x_ : agents_[agentsKeys_[left]].position_.y_) < splitValue)
                    {
                        ++left;
                    }

					while (right > left && (isVertical ? agents_[agentsKeys_[right - 1]].position_.x_ : agents_[agentsKeys_[right - 1]].position_.y_) >= splitValue)
                    {
                        --right;
                    }
                    ...
                    if (left < right)
                    {
						Agent tempAgent = agents_[agentsKeys_[left]];
						agents_[agentsKeys_[left]] = agents_[agentsKeys_[right - 1]];
						agents_[agentsKeys_[right - 1]] = tempAgent;
                        ++left;
                        --right;
                    }
                }
...

            {
                ...
                for (int j = 0; j < obstacles.Count; ++j)
                {
                    ...
                    else
                    {
                        ...
                        newObstacle.id_ = Simulator.Instance.newObstacleID_++; //Simulator.Instance.obstacles_.Count;

                        Simulator.Instance.obstacles_.Add(newObstacle.id_, newObstacle);

                        obstacleJ1.splitted_ = true;
                        obstacleJ1.splitObstacleId_ = newObstacle.id_;
        ...
        private void queryAgentTreeRecursive(Agent agent, ref float rangeSq, int node)
        {
            if (agentTree_[node].end_ - agentTree_[node].begin_ <= MAX_LEAF_SIZE)
            {
                ...
                {
					agent.insertAgentNeighbor(agents_[agentsKeys_[i]], ref rangeSq);
                }
            }
            else
            {
